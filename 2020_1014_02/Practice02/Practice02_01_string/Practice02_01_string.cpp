...
int main()
{
    // C言語の文字、文字列
    char word = 'a';                  // 一文字
    char name[] = "ラーメン食べたい"; // 文字配列
    const char* cstr = "Hello World"; // const char ポインタ型

    // 上記三つの文字、文字列をprintfで画面に出力する
    printf("%c\n", word);
    printf("%s\n", name);
    printf("%s\n", cstr);
...
//    printf("%s\n", str.c_str());
    ...
    str.append("EE");   // "AAABBCD" 末尾に文字列を追加
    str.insert(1,"FG"); // "AFGAABBCD"指定の場所に文字列を追加
    std::cout << str << std::endl;

    // 文字の検索 .find()
    str = "ABC ABC";
    int index = str.find('C');    // 2 先頭から1文字を探す
...
//    .find_last_not_of() 指定文字でない文字を最後から検索
    ...
    // 文字の置き換え .replace(何文字目、何文字分、置き替え文字列)
    str = "ABC ABC";
    std::string from = "BC";
    std::string to = "X";
